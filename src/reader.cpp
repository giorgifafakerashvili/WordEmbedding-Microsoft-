...
Reader::Reader(Dictionary* dictionary, Option* option, 
	...
	if(option_->stopwords) {
		FILE* fid = fopen(option->sw_file, "r");
		if(fid == nullptr) {
...
int Reader::GetSentence(int* sentence, int64_t& word_count) {
	...
	while(1) {
		...
		if(option_->stopwords && stopwords_table_.count(word_)) 
			continue;
		if(option_->sample > 0 && 
			!sampler_->WordSampling(dictionary_->GetWordInfo(word_idx)->freq, 
				options_->total_words, option_->sample))
			continue;
...
void Reader::ResetSize(int64_t size) {
	byte_count_ = 0;
	byte_size_ = size;
}
